...
public class SessionTree {
	...
	public SessionTree(TreeNode rootData) {
	...
	public JsonObject TreeToJson(TreeNode node) {
		...
		if (node.getKey().equals("datasetlist")) {
			...
			/*
			 * TreeNode parentNode = node.getParent(); if
			 * (parentNode.getKey().equals("datasetlist")) { Map<String, String>
			 * parentFilter = parentNode.getFilter(); Map<String, String> filter
			 * = node.getFilter();
			 * 
			 * String addFilter = ""; String delFilter = ""; for (String key :
			 * filter.keySet()) { if (!parentFilter.containsKey(key)) {
			 * addFilter += key + "=" + filter.get(key); } }
			 * 
			 * for (String key : parentFilter.keySet()) { if
			 * (!filter.containsKey(key)) { delFilter += key + "=" +
			 * parentFilter.get(key); } }
			 * 
			 * if (!addFilter.equals("")) { json.addProperty("name", "+" +
			 * addFilter); }
			 * 
			 * if (!delFilter.equals("")) { json.addProperty("name", "-" +
			 * delFilter); }
			 * 
			 * } else if (parentNode.getKey().equals("root")) {
			 * json.addProperty("name", "+" + node.getFilterStr()); }
			 * 
			 * json.addProperty("tips", node.());
			 * 
			 */
	...
	public List<ClickThroughData> genClickThroughData(List<TreeNode> viewnodes) throws UnsupportedEncodingException {
		...
		for (int i = 0; i < viewnodes.size(); i++) {
			...
			String query = RequestUrl.GetSearchInfo(parent.getRequest());
			...
			if (!viewquery.equals("")) {
				...
				List<String> viewqueries = new ArrayList<String>();
				if(viewquery.contains(",")){
					viewqueries = Arrays.asList(viewquery.split(","));
				}else{
					viewqueries.add(viewquery);
				}
				
				for(int z=0; z<viewqueries.size(); z++){
				ClickThroughData data = new ClickThroughData(viewqueries.get(z), dataset, download);
